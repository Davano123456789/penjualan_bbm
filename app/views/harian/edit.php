<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Laporan Harian</h2>
    ...
    <!-- Main Entry Form -->
    <form action="<?= BASEURL; ?>/harian/ubah" method="POST" class="space-y-8 pb-20">
        <input type="hidden" name="id" value="<?= $data['harian']['id']; ?>">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div>
                ...
                <input type="date" name="tanggal" value="<?= $data['harian']['tanggal']; ?>" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 transition-all">
            </div>
            <div>
                ...
                <select name="operator1_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="<?= $op['id']; ?>" <?= ($op['id'] == $data['harian']['operator1_id']) ? 'selected' : ''; ?>><?= $op['nama']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                ...
                <select name="operator2_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500">
                    ...
                    <?php foreach($data['operators'] as $op) : ?>
                    <option value="<?= $op['id']; ?>" <?= ($op['id'] == $data['harian']['operator2_id']) ? 'selected' : ''; ?>><?= $op['nama']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="jam_masuk" id="jam_masuk" value="<?= $data['harian']['jam_masuk']; ?>" placeholder="21.00" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-center font-bold focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-2 text-center">Keluar</label>
                    <input type="text" name="jam_keluar" id="jam_keluar" value="<?= $data['harian']['jam_keluar']; ?>" placeholder="06.00" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-center font-bold focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left" id="readingTable">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        // Index penjualan by nozzle for easy access
                        $penjualanIndexed = [];
                        foreach ($data['penjualan'] as $pj) {
                            $penjualanIndexed[$pj['nozzle']] = $pj;
                        }
                        ...
                        foreach ($machines as $mIdx => $m) : 
                            for ($i = 0; $i < count($m['nozzles']); $i += 2) :
                                ...
                                $p1 = $penjualanIndexed[$n1] ?? ['produk_id' => '', 'totalisator_awal' => 0, 'totalisator_akhir' => 0, 'liter_terjual' => 0, 'total_rupiah' => 0, 'harga_jual' => 0];
                                $p2 = $penjualanIndexed[$n2] ?? ['produk_id' => '', 'totalisator_awal' => 0, 'totalisator_akhir' => 0, 'liter_terjual' => 0, 'total_rupiah' => 0, 'harga_jual' => 0];

                                // Use price from one of the nozzles as group price
                                $groupHarga = $p1['harga_jual'] ?: ($p2['harga_jual'] ?: 0);
                        ?>
                        ...
                        <tr class="row-reading group" data-group="<?= $groupId; ?>">
                            ...
                            <td class="px-6 py-3">
                                <select name="penjualan[<?= $n1; ?>][produk_id]" class="produk-select w-40 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm transition-all focus:border-blue-500">
                                    ...
                                    <option value="<?= $p['id']; ?>" data-harga="<?= $p['harga_jual']; ?>" <?= ($p['id'] == $p1['produk_id']) ? 'selected' : ''; ?>><?= $p['nama']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </td>
                            <td class="px-4 py-3">
                                <input type="number" step="0.01" name="penjualan[<?= $n1; ?>][awal]" value="<?= $p1['totalisator_awal']; ?>" class="awal w-24 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-right focus:border-blue-500">
                            </td>
                            <td class="px-4 py-3">
                                <input type="number" step="0.01" name="penjualan[<?= $n1; ?>][akhir]" value="<?= $p1['totalisator_akhir']; ?>" class="akhir w-24 px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-right font-bold focus:ring-2 focus:ring-blue-500">
                            </td>
                            <td class="px-6 py-3">
                                <input type="number" step="0.01" readonly name="penjualan[<?= $n1; ?>][liter_terjual]" value="<?= $p1['liter_terjual']; ?>" class="liter-individual w-20 px-3 py-2 bg-slate-100 border border-transparent rounded-lg text-sm text-right font-bold text-slate-500" value="0">
                            </td>
                            ...
                            <td rowspan="2" class="px-6 py-3 bg-blue-50/10">
                                <input type="number" name="group_data[<?= $groupId; ?>][harga]" value="<?= $groupHarga; ?>" class="group-harga w-24 px-3 py-3 bg-slate-50 border border-slate-200 rounded-xl text-center text-sm font-bold text-emerald-600 focus:ring-2 focus:ring-emerald-500">
                            </td>
                            <td rowspan="2" class="px-6 py-3 bg-blue-50/10 text-right">
                                ...
                                <input type="hidden" name="penjualan[<?= $n1; ?>][total_rupiah]" class="nozzle-rupiah-share" value="<?= $p1['total_rupiah']; ?>">
                                <input type="hidden" name="penjualan[<?= $n2; ?>][total_rupiah]" class="nozzle-rupiah-share" value="<?= $p2['total_rupiah']; ?>">
                        ...
                        <tr class="row-reading group border-b border-slate-100" data-group="<?= $groupId; ?>">
                            ...
                            <td class="px-6 py-3">
                                <select name="penjualan[<?= $n2; ?>][produk_id]" class="produk-select w-40 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm transition-all focus:border-blue-500">
                                    <option value="">- Produk -</option>
                                    <?php foreach ($data['produk'] as $p) : ?>
                                    <option value="<?= $p['id']; ?>" data-harga="<?= $p['harga_jual']; ?>" <?= ($p['id'] == $p2['produk_id']) ? 'selected' : ''; ?>><?= $p['nama']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </td>
                            <td class="px-4 py-3">
                                <input type="number" step="0.01" name="penjualan[<?= $n2; ?>][awal]" value="<?= $p2['totalisator_awal']; ?>" class="awal w-24 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-right focus:border-blue-500">
                            </td>
                            <td class="px-4 py-3">
                                <input type="number" step="0.01" name="penjualan[<?= $n2; ?>][akhir]" value="<?= $p2['totalisator_akhir']; ?>" class="akhir w-24 px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-right font-bold focus:ring-2 focus:ring-blue-500">
                            </td>
                            <td class="px-6 py-3">
                                <input type="number" step="0.01" readonly name="penjualan[<?= $n2; ?>][liter_terjual]" value="<?= $p2['liter_terjual']; ?>" class="liter-individual w-20 px-3 py-2 bg-slate-100 border border-transparent rounded-lg text-sm text-right font-bold text-slate-500" value="0">
                            </td>
                        </tr>
                        <?php 
                            endfor;
            ...
            <div class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
                ...
                <div class="space-y-5 relative z-10">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <input type="number" name="total_titipan" id="titipan" value="<?= $data['harian']['total_titipan']; ?>" class="w-44 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-right font-bold focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-bold text-rose-500 uppercase">3. Pengeluaran</span>
                        <input type="number" name="total_pengeluaran" id="pengeluaran" value="<?= $data['harian']['total_pengeluaran']; ?>" class="w-44 px-4 py-3 bg-rose-50 border border-rose-100 rounded-xl text-sm text-right font-bold text-rose-600 focus:ring-2 focus:ring-rose-500 transition-all">
                    </div>
                ...
                <div class="space-y-4 relative z-10">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" id="actual_kertas" value="<?= $data['harian']['total_penerimaan_kas']; ?>" placeholder="Rp..." class="w-full px-4 py-3 bg-slate-800 border-none rounded-xl text-md font-bold text-emerald-400 focus:ring-2 focus:ring-emerald-500 transition-all shadow-inner">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-2">Koin (Logam)</label>
                            <input type="number" id="actual_koin_fisik" value="0" placeholder="Rp..." class="w-full px-4 py-3 bg-slate-800 border-none rounded-xl text-md font-bold text-emerald-400 focus:ring-2 focus:ring-emerald-500 transition-all shadow-inner">
                        </div>
                    </div>
                    <div class="pt-4 border-t border-slate-800">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-2">Total Kas Fisik (Di Tangan)</label>
                        <input type="number" readonly name="total_penerimaan_kas" value="<?= $data['harian']['total_penerimaan_kas']; ?>" id="actual_cash_total" class="w-full px-5 py-4 bg-slate-700/50 border-none rounded-2xl text-2xl font-black text-white shadow-inner">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-emerald-50/50 rounded-3xl border-2 border-dashed border-emerald-200 p-8 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-white p-5 rounded-2xl border border-emerald-100 shadow-sm">
                    ...
                    <input type="number" step="0.01" name="stok_fisik_pertamax" value="<?= $data['stok_fisik_pertamax']; ?>" placeholder="0.00" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-black focus:ring-2 focus:ring-emerald-500">
                </div>
                <div class="bg-white p-5 rounded-2xl border border-emerald-100 shadow-sm">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Sisa Dex di Tangki (Liter)</label>
                    <input type="number" step="0.01" name="stok_fisik_dex" value="<?= $data['stok_fisik_dex']; ?>" placeholder="0.00" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-black focus:ring-2 focus:ring-emerald-500">
                </div>
            </div>
        </div>
        ...
        <div class="fixed bottom-0 left-64 right-0 bg-white/80 backdrop-blur-md p-4 border-t border-slate-100 shadow-xl flex justify-end px-10">
            <input type="hidden" name="total_sisa" id="hidden_sisa_output" value="<?= $data['harian']['total_sisa']; ?>">
            <button type="submit" class="px-8 py-3 bg-blue-600 text-white rounded-xl font-black text-md hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition-all flex items-center gap-3">
                <i class="fas fa-save"></i> UPDATE LAPORAN HARIAN
            </button>
        </div>
    </form>
</div>
